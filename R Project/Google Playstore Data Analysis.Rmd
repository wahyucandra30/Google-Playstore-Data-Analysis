...
Untuk memulai proses binerisasi kolom installs, perlu dilakukan konversi data installs dari char ke numerik. Pertama, kami hapus tanda **+** dan **,** untuk memudahkan konversi ke numerik.
```{r}
...
```{r}
#df[c("Installs")] <- as.numeric(df[c("Installs")])
#df[c("Installs")] <- ifelse(df[c("Installs")] > 50000, 1, 0)
```

```{r}
#set.seed(666) # Menyimpan seed agar sample dapat dibuat ulang.
